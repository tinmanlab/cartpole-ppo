...
const RobustV2=(()=>{
 ...
 const DEFAULT_HP={gamma:.99,lambda:.95,epsilon:.2,actorLR:.0003,criticLR:.001,entropy:.002,epochs:4,batch:128,n:16,horizon:128,history:HISTORY_STEPS,plant:{...P.DEFAULT_SPEC},gateEvery:5,domainRandomizationProb:.25,stratifiedFamilies:true,foundationCurriculum:true,foundationGateStreak:2};
 ...
 class Trainer{
  constructor(seed=123,hp={}){
   ...
   if(typeof this.hp.foundationCurriculum!=='boolean'||!Number.isInteger(this.hp.foundationGateStreak)||this.hp.foundationGateStreak<1)throw Error('Invalid foundation curriculum.');
   this.foundation={active:this.hp.foundationCurriculum,streak:0,history:[]};
  ...
  _laneFor(i){return !this.hp.stratifiedFamilies?null:this.foundation.active?'nominal':STRATIFIED_FAMILIES[i];}
  _makeSlot(i){const env=new CP(new Random((this.seed+1009+i*7919)>>>0),{spec:this.hp.plant,profile:'nominal',seed:(this.seed+700001+i*3571)>>>0}),lane=this._laneFor(i),slot={env,history:null,schedule:null,prevAction:0,family:lane||'nominal',lane,domainRandomized:false};this._resetSlot(slot,lane);return slot;}
  _activateRobustLanes(){if(!this.hp.stratifiedFamilies)return;for(let i=0;i<this.slots.length;i++){this.slots[i].lane=STRATIFIED_FAMILIES[i];this._resetSlot(this.slots[i],this.slots[i].lane);}}
  _gradeFoundation(nominal,count=8){if(!this.foundation.active)return {event:'foundation_inactive',nominal,count};this.foundation.streak=nominal>=Math.ceil(.875*count)?this.foundation.streak+1:0;let event='foundation_hold';if(this.foundation.streak>=this.hp.foundationGateStreak){this.foundation.active=false;event='foundation_complete';this._activateRobustLanes();}const row={iter:this.iter,nominal,count,streak:this.foundation.streak,event};this.foundation.history.push(row);if(this.foundation.history.length>100)this.foundation.history.shift();return row;}
  ...
  gate(){const base=(this.seed+900000+this.iter*1009)>>>0,nom=evaluateGateFamily(this.snapshot(),'nominal',this.boundary.value,8,base);if(this.foundation.active){const grade=this._gradeFoundation(nom,8);return {...grade,phase:'foundation',nominal:nom,tip:0,mixed:0,boundary:this.boundary.value};}const tip=evaluateGateFamily(this.snapshot(),'tip',this.boundary.value,8,base+10000),mix=evaluateGateFamily(this.snapshot(),'mixed',this.boundary.value,8,base+20000),grade=this.boundary.grade({nominal:nom,tip,mixed:mix,count:8});return {...grade,phase:'robust',nominal:nom,tip,mixed:mix};}
  snapshot(){return {schema:'cartpole-robust-v2-policy/v1',method:'robust-v2',iter:this.iter,plant:clone(this.hp.plant),hp:clone(this.hp),foundation:clone(this.foundation),boundary:this.boundary.snapshot(),actor:this.actor.snapshot(),critic:this.critic.snapshot()};}
  checkpoint(){return {schema:'cartpole-robust-v2-checkpoint/v1',seed:this.seed,hp:clone(this.hp),iter:this.iter,steps:this.steps,episodes:this.episodes,scores:this.scores.slice(),initial:this.initial.slice(),last:clone(this.last),gateResult:clone(this.gateResult),rng:rngState(this.rng),foundation:clone(this.foundation),boundary:this.boundary.snapshot(),actor:this.actor.optimizerSnapshot(),critic:this.critic.optimizerSnapshot(),slots:this.slots.map(s=>{const {rng,noiseRng,...env}=s.env;return {env:clone(env),rng:rngState(rng),noiseRng:rngState(noiseRng),history:s.history.snapshot(),schedule:clone(s.schedule),prevAction:s.prevAction,family:s.family,lane:s.lane,domainRandomized:s.domainRandomized};})};}
 }

 function restoreTrainer(cp){
  if(cp?.schema!=='cartpole-robust-v2-checkpoint/v1')throw Error('Robust PPO v2 requires its own v1 checkpoint.');const hp=clone(cp.hp);if(!Object.hasOwn(hp,'domainRandomizationProb'))hp.domainRandomizationProb=0;if(!Object.hasOwn(hp,'stratifiedFamilies'))hp.stratifiedFamilies=false;if(!Object.hasOwn(hp,'foundationCurriculum'))hp.foundationCurriculum=false;if(!Object.hasOwn(hp,'foundationGateStreak'))hp.foundationGateStreak=2;const t=new Trainer(cp.seed,hp);t.iter=cp.iter;t.steps=cp.steps;t.episodes=cp.episodes;t.scores=cp.scores.slice();t.initial=cp.initial.slice();t.last=clone(cp.last);t.gateResult=clone(cp.gateResult);t.rng=restoreRng(cp.rng);t.foundation=cp.foundation?clone(cp.foundation):{active:false,streak:0,history:[]};t.boundary=new AdaptiveBoundary(cp.boundary);t.actor=NN.from(cp.actor);t.critic=NN.from(cp.critic);
