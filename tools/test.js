...
for(const file of ['test_public_contract.js','test_bam.js','test_lesson.js','test_integration.js']){
 const r=spawnSync(process.execPath,[path.join(root,'tests',file)],{cwd:root,stdio:'inherit'});
 if(r.error)throw r.error;
 if(r.status!==0)process.exit(r.status||1);
}
console.log('All four engine/contract suites passed. Browser tests are separate.');
