...
with sync_playwright() as pw:
 ...
 check('Policy/test mismatch remains explicit', 'outside' in p.locator('#conditionbar').inner_text())
 ...
 p.select_option('#recordSource','example');p.click('[data-chapter="2"]')
 ...
 colors=p.eval_on_selector_all('.network-svg line[data-encoded]','es=>Array.from(new Set(es.map(e=>e.getAttribute("stroke"))))')
 ...
 p.check('#netLive');p.click('#matchPolicyWorld');p.click('[data-chapter="2"]');p.check('#netLive') if not p.locator('#netLive').is_checked() else None
 ...
 p.select_option('#pushForce','4');p.click('#pushRight');p.wait_for_function('PPOStep.status().userForce===4',timeout=2000)
 check('Manual push is actual time-bounded external force',status(p)['userForce']==4)
 p.wait_for_timeout(500);check('Pulse expires after its specified duration',status(p)['pulseSteps']==0)
 ...
 p.select_option('#recordSource','example');p.click('[data-chapter="2"]');p.select_option('#netMode','activation');p.click('[data-forward="1"]');p.screenshot(path=str(root/'evidence/preview.png'))
